...
-- return the rgb value as a 3 digit number with each digit between 0-5
local function _color_index(rgb)
  local least_distance = math.huge
  local closestRgb = {}
  for name, color in pairs(color_table) do
    local color_distance = math.sqrt((color[1] - rgb[1]) ^ 2 + (color[2] - rgb[2]) ^ 2 + (color[3] - rgb[3]) ^ 2)
  ...
  local function _compute_idx(value)
    if value then
      local retval = (value - 55) / 40
      if retval > 0 then
        return retval
      else
        return 0
  ...
  local idx = string.format("%d%d%d", _compute_idx(closestRgb[1]), _compute_idx(closestRgb[2]),
    _compute_idx(closestRgb[3]))
...
local function errorIfEmpty(text, funcName)
  assert(#text > 0,
    string.format("%s: you passed in an empty string, and I cannot make a gradient out of an empty string", funcName))
end
...
local function dgradient(text, ...)
  ...
  local current_color = ""
  for character = 1, #text do
    local new_color = "<" .. table.concat(_color_index(gradients[character]), ",") .. ">"
...
-- gradients for Nukefire and other Circle/tbaMUD muds
local function nfgradient(text, ...)
  errorIfEmpty(text, "nfgradient")
  local gradients = _gradients(#text, ...)
  local dgrad = ""
  local current_color = ""
  for character = 1, #text do
    local new_color = "@[f" .. _color_index(gradients[character]) .. "]"
    local char = text:sub(character, character)
    if new_color == current_color then
      dgrad = dgrad .. char
    else
...
-- @see hgradient
-- @usage
-- decho(GradientMaker.nfgradient("a luminescent butterly floats about lazily on brillant blue and lilac wings\n", {255,0,0}, {255,128,0}, {255,255,0}, {0,255,0}, {0,255,255}, {0,128,255}, {128,0,255}))
-- decho(GradientMaker.nfgradient("a luminescent butterly floats about lazily on brillant blue and lilac wings\n", {255,0,0}, {0,0,255}))
-- decho(GradientMaker.nfgradient("a luminescent butterly floats about lazily on brillant blue and lilac wings\n", {50,50,50}, {0,255,0}, {50,50,50}))
function GradientMaker.nfgradient(text, ...)
  return nfgradient(text, ...)
end
